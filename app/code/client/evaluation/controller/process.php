...
Class Evaluation_Controller_Process extends Frontend_Controller_Action {
	...
	public function submitAction() {
		...
		$ratingDb->insert([
			...
			"crit_A4" => $request[7],
			"crit_A5" => $request[8],
			"ave_crit1" => $this->getAve([$request[1],$request[3],$request[6],$request[7],$request[8]]),
			...
			"crit_B4" => $request[11],
			"crit_B5" => $request[12],
			"ave_crit2" => $this->getAve([$request[2],$request[9],$request[10],$request[11],$request[12]]),
			...
			"crit_C4" => $request[15],
			"crit_C5" => $request[16],
			"ave_crit3" => $this->getAve([$request[4],$request[13],$request[14],$request[15],$request[16]]),
			...
			"crit_D4" => $request[19],
			"crit_D5" => $request[20],
			"ave_crit4" => $this->getAve([$request[5],$request[17],$request[18],$request[19],$request[20]]),

			"ave_total" => $this->getAve([
				$this->getAve([$request[1],$request[3],$request[6],$request[7],$request[8]]),
				$this->getAve([$request[2],$request[9],$request[10],$request[11],$request[12]]),
				$this->getAve([$request[4],$request[13],$request[14],$request[15],$request[16]]),
				$this->getAve([$request[5],$request[17],$request[18],$request[19],$request[20]]),
			])
		]);
	...
	public function getAve($ans = array()) {
		$ave = 0;
		foreach( $ans as $an ) {
			$ave = $ave + $an;
		}

		return $ave / count($ans);
	}
